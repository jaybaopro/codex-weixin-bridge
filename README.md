...
- 公开链接只允许 `https://mp.weixin.qq.com` 的单篇公众号文章；桥接器无登录、
  无 Cookie 抓取正文，Codex 自身的通用网络能力仍然关闭。
- 公众号链接会移除分享跟踪参数，重定向目标仍须属于同一精确域名；文章图片不会下载
  或 OCR，网页正文按不可信只读数据处理。
...
在 ClawBot 对话中，把文章的公开 URL 作为文字粘贴发送即可，例如：
...
当前每条消息只读取一篇文章，也不能把公众号链接和附件放在同一条消息中。支持
`/s/<文章短码>` 和带完整文章定位参数的 `/s?...` 链接；分享场景、票据等跟踪参数
会在下载和交给 Codex 前移除。

读取范围只包括页面中的公开文字正文、标题、公众号名、作者和可识别的发布日期。
正文图片不会下载或 OCR；以长图为主的文章会明确提示当前无法读取。登录后文章、
已删除文章、验证页、非公众号域名、HTTP 链接、跨域跳转、超过 8 MB 的页面或
超过 25 万字符的正文会整篇拒绝，不做静默截断。

这项能力是桥接器内的一条窄网络通道，不会为当前 Codex 任务开启 Web 搜索、任意
网页访问、应用或 MCP。文章正文即使包含针对 AI 的指令，也只会作为待分析资料，
不能触发任务切换、审批、项目外读取或权限扩大。
